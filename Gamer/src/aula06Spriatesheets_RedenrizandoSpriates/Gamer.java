...
e implementamos o metodo runnable*/
	...
	private SpriteSheet sheet; // chamando nossa classe, e criando um objetoc sheet
	private BufferedImage player; // chamando nossa classe SpriteSheet para criar um objeto
	private int x = 0;
	
	public Gamer() { // nosso construtor da classe GAMER
		sheet = new SpriteSheet("/sprintSheet.png"); // carregando nossa imagem
		player = sheet.getSprite(0, 0, 15, 16);
 ...
 /*     redenrização do jogo neste metodo abaixo**/

		g.drawImage(player, x, 50, null);// a imagem, posiçao em X e en Y
		g.drawImage(player, x, 30, null);// redenrizando a nossa imagem
		g.drawImage(player, 50, 120, null);// quantas vezes formo querer
